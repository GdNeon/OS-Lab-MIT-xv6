...
pagetable_t proc_k_pagetable()
{
  ...
  if(mappages(p, VIRTIO0, PGSIZE, VIRTIO0, PTE_R | PTE_W) != 0) return 0;
  if(mappages(p, PLIC, 0x400000, PLIC, PTE_R | PTE_W) != 0) return 0;
  if(mappages(p, KERNBASE, (uint64)etext-KERNBASE, KERNBASE, PTE_R | PTE_X) != 0) return 0;
  if(mappages(p, (uint64)etext, PHYSTOP-(uint64)etext, (uint64)etext, PTE_R | PTE_W) != 0) return 0;
  if(mappages(p, TRAMPOLINE, PGSIZE, (uint64)trampoline, PTE_R | PTE_X) != 0) return 0;
  return p;
}
...
void copypagetable(pagetable_t upagetable, pagetable_t kpagetable, uint64 sz0, uint64 sz1)
{
  ...
  for (uint64 i = sz0; i < sz1; i += PGSIZE)
  {
    ...
    if (pte_u == 0) panic("copypagetable: fail to copy!");
    if (pte_k == 0) panic("copypagetable: fail to paste!");
