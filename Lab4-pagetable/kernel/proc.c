...
found:
  ...
  if (mappages(p->k_pagetable, (uint64)p->kstack, PGSIZE, (uint64)p->pakstack, PTE_R | PTE_W) != 0) 
  {
      freeproc(p);
      release(&p->lock);
      return 0;
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    copypagetable(p->pagetable, p->k_pagetable, p->sz, sz);
  } else if(n < 0){
...
fork(void)
{
  ...
  copypagetable(np->pagetable, np->k_pagetable, 0, np->sz);
